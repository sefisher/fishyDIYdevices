...
//Define Device Name
const char* device = CUSTOM_DEVICE_NAME;
...
enum trueState {opening, opened, closing, closed, man_idle, man_CCW, man_CW, unknown}; //used for internal state tracking
static const char *trueState_String[] = {"opening", "opened", "closing", "closed", "man_idle", "man_CCW", "man_CW", "unknown"};
enum calStages {doneCal, openingCal, closingCal};
...
int currentPos;
enum manCommandState {none, CW, CCW, SEL}; //used for tracking the commanded state with manual controls
...
void WiFiSetup() {
	...
	WiFiManager.autoConnect(CUSTOM_DEVICE_NAME); 
...
void setup() {
	...
	*/
	...
	if(MASTER_NODE){ UDPannounceMaster();}
}
...
//first verifies this is the MASTER_NODE since other nodes have a zero size array
...
fishyDevice makeMyFishyDevice(){
	...
	if(motorPosAtCCWset && motorPosAtCWset) {holder.isCalibrated = true;} else {holder.isCalibrated = false;}
	holder.isMaster = MASTER_NODE;
	...
	holder.isMaster = MASTER_NODE;
	if(OPEN_IS_CCW_OR_CW == "CCW") {holder.openIsCCW = true;} else {holder.openIsCCW = false;} 
...
void UDPpollReply(IPAddress remote){
	...
	if(MASTER_NODE){UDPannounceMaster();} //make sure they know who is in charge
...
void UDPparsePollResponse(String responseIn,IPAddress remote){
	if(MASTER_NODE){
		...
		if(DEBUG_MESSAGES && UDP_PARSE_MESSAGES){ Serial.println("[UDPparsePollResponse] strIP: " + holder.ip.toString());}
		...
		if(DEBUG_MESSAGES && UDP_PARSE_MESSAGES){ Serial.println("[UDPparsePollResponse] motorPos: " + String(holder.motorPos));}

		//motorPosAtCCW
		strStrt = strStop+1;
		strStop = response.indexOf(",",strStrt);
		holder.motorPosAtCCW = response.substring(strStrt,strStop).toInt();
		if(DEBUG_MESSAGES && UDP_PARSE_MESSAGES){ Serial.println("[UDPparsePollResponse] motorPosAtCCW: " + String(holder.motorPosAtCCW));}
		
		//motorPosAtCW
		strStrt = strStop+1;
		strStop = response.indexOf(",",strStrt);
		holder.motorPosAtCW = response.substring(strStrt,strStop).toInt();
		if(DEBUG_MESSAGES && UDP_PARSE_MESSAGES){ Serial.println("[UDPparsePollResponse] strMotorPosAtCW: " + String(holder.motorPosAtCW));}
		
		//motorPosAtFullCCW
		strStrt = strStop+1;
		strStop = response.indexOf(",",strStrt);
		holder.motorPosAtFullCCW = response.substring(strStrt,strStop).toInt();
		if(DEBUG_MESSAGES && UDP_PARSE_MESSAGES){ Serial.println("[UDPparsePollResponse] strMotorPosAtFullCCW: " + String(holder.motorPosAtFullCCW));}
		
		//motorPosAtFullCW
		strStrt = strStop+1;
		strStop = response.indexOf(",",strStrt);
		holder.motorPosAtFullCW = response.substring(strStrt,strStop).toInt();
		if(DEBUG_MESSAGES && UDP_PARSE_MESSAGES){ Serial.println("[UDPparsePollResponse] strMotorPosAtFullCW: " + String(holder.motorPosAtFullCW));}
		
		//name
		strStrt = strStop+1;
		strStop = response.indexOf(",",strStrt);
		holder.name = response.substring(strStrt,strStop);
		if(DEBUG_MESSAGES && UDP_PARSE_MESSAGES){ Serial.println("[UDPparsePollResponse] strName: " + holder.name);}
		...
		if(DEBUG_MESSAGES && UDP_PARSE_MESSAGES){ Serial.println("[UDPparsePollResponse] strPort: " + String(holder.port));}
...
void manCW() {
	...
	if (CWlimSensorVal == 1) { //not at CW limit
...
void manCCW() {
	...
	if (CCWlimSensorVal == 1) { //not at CCW limit
...
void manSel() {
	...
	case none:
	case SEL:
		if (manSelCnt > 5) { //select button held long time - reset the limits and put the deviceState -> unknown
			if (DEBUG_MESSAGES) {
...
void setCWLimit(long newLimit) {
	...
	if (motorPosAtCCW >= motorPosAtCW) { //if limits are reversed, update the CCW limit
		if (DEBUG_MESSAGES) {
...
void setCCWLimit(long newLimit) {
	...
	motorPosAtCCWset = true;
	if (motorPosAtCW <= motorPosAtCCW) { //if limits are reversed, update the CW limit
		if (DEBUG_MESSAGES)
		{
...
void normalOpening() {
	if (!digitalRead(SWpinLimitCCW))
	{
		...
		if (DEBUG_MESSAGES)
		{
			int CCWlimSensorVal = digitalRead(SWpinLimitCCW); int CWlimSensorVal = digitalRead(SWpinLimitCW);
...
void dataDump(){
	...
	sprintf(temp,"[DATA DUMP]\nSwitches -     LimCCW=%d LimCW=%d; ManCCW=%d ManCW=%d ManSEL=%d\nMotor -        "
				...
				,CCWlimSensorVal,CWlimSensorVal,CCWmanSensorVal,CWmanSensorVal,SELmanSensorVal, curPos,stepper1.isRunning()?"Y":"N",curSpeed,distToGo
				,motorPosAtCCWset? "Y" : "N",motorPosAtCCW,motorPosAtCWset? "Y" : "N",motorPosAtCW
				,deviceState ?"ON":"OFF",trueState_String[deviceTrueState],calStages_String[deviceCalStage]);
...
void loop() {
	...
	if ((CWmanSensorVal + CCWmanSensorVal + SELmanSensorVal) == 3) { //New MANUAL CONTROL not ordered - all three are HIGH (switches open)
		...
		case doneCal:
			...
			case man_CCW: //opening manually
				if (CCWmanSensorVal == 1) { //was manually opening, now done
			...
			case man_CW: //closing manually
				if (CWmanSensorVal == 1) { //was manually closing, now done
					idleActuator(man_idle);
				}
				else
				{ //run
		...
		case openingCal: //calibration in first stage
			normalOpening();
			if (deviceTrueState == opened) { //Done with stage 1 go onto closing stage
		...
		case closingCal: //calibration in second stage
			normalClosing();
			if (deviceTrueState == closed) { //Done with stage 2 - done!
...
void executeCommands(char inputMsg[20],IPAddress remote){
	...
	if (cmd.startsWith("open")) { //TODO - make open determine CCW or CW
		if (DEBUG_MESSAGES) {Serial.println("[executeCommands] Commanded OPEN");}
		executeState(true); //WiFi "true" is open or going CCW to stop
	} else if (cmd.startsWith("close")) {  //TODO - make open determine CCW or CW
		Serial.println("[executeCommands] Commanded CLOSE");
		executeState(false); //WiFi "false" is close or going CW to stop
	} else if (cmd.startsWith("goto")) {  //TODO - make open determine CCW or CW
		Serial.println("[executeCommands] Commanded GOTO (" + cmd + ")");
		executeGoto(cmd); //WiFi "false" is close or going CW to stop
	} else if (cmd.startsWith("data")) {
		if (DEBUG_MESSAGES) {Serial.println("[executeCommands] Commanded DATA");}
		//TODO - make it work for serial also
		//dataDump();
	} else if (cmd.startsWith("reset")) {
		if (DEBUG_MESSAGES) {Serial.println("[executeCommands] Commanded RESET");}
		resetController();
	} else if(cmd.startsWith("memory")){
		readEEPROM();
	} else if (cmd.startsWith("calibrate")) { //TODO - rename this "test full range"
		if (DEBUG_MESSAGES) {Serial.println("[executeCommands] Commanded CALIBRATE");}
		deviceCalStage = openingCal;
		if (DEBUG_MESSAGES) {Serial.printf("[executeCommands] Going to calibration opening stage.\n");}
		executeState(true);
	} else if (cmd.startsWith("anyfishydev_there")){
		if (DEBUG_MESSAGES) {Serial.println("[executeCommands] Commanded ANYFISHYDEV_THERE");}
		UDPpollReply(remote);
	} else if (cmd.startsWith("poll_net")){
		if (DEBUG_MESSAGES) {Serial.println("[executeCommands] Commanded POLL_NET");}
		UDPbroadcast();
	} else if (cmd.startsWith("poll_response")){
		if (DEBUG_MESSAGES) {Serial.println("[executeCommands] Commanded POLL_RESPONSE");}
		UDPparsePollResponse(String(inputMsg),remote); //want the original case for this
	} else if (cmd.startsWith("fishydiymaster")){ 
		if (DEBUG_MESSAGES) {Serial.println("[executeCommands] Commanded FISHYDIYMASTER");}
...
trueState idleActuator(trueState idleState) {
	...
	UDPpollReply(masterIP); //tell the MASTER_NODE thhe new info
	return idleState;
}

trueState openActuator() {
	trueState newState;
	if (digitalRead(SWpinLimitCCW) == 1) { //switch is open; not at CCW limit
		newState = opening;
		if (DEBUG_MESSAGES) {
...
trueState openPercentActuator(int percent) {
	...
	if (newMotorPos < stepper1.currentPosition()){ //NEED TO MOVE CCW
		if (digitalRead(SWpinLimitCCW) == 1) { //switch is open; not at CCW limit
			newState = opening;
			if (DEBUG_MESSAGES) {
	...
	if (digitalRead(SWpinLimitCW) == 1) { //switch is open; not at CW limit
		newState = closing;
		if (DEBUG_MESSAGES) {
	...
	for(int i=0;i<MAX_DEVICE;i++){
		if(!deviceArray[i].dead){
			...
			temp += "{\"deviceID\":\"" + String(i) + "\",\"IP\":\"" + deviceArray[i].ip.toString() + "\",\"dead\":\"" + String(deviceArray[i].dead) + 
			 ...
			 "\",\"port\":\"" + String(deviceArray[i].port) + "\"}";
		}
	}
	temp+="]}";
...
void handleGenericArgs() { //Handler
	...
	for (int i = 0; i < httpServer.args(); i++) {
		...
		if(httpServer.argName(i)=="IPCommand"){
			...
			if(DEBUG_MESSAGES && UDP_PARSE_MESSAGES){ Serial.println("[handleGenericArgs] strIP:response " + IPtoSend.toString() + ":" + response);}
...
void handleRoot(){
	if(DEBUG_MESSAGES){Serial.println("[handleRoot] 1: ");}
		
	int szchnk = 2900;
	
	
	//LARGE STRINGS - break transmission into parts
	...
	//Send WEBSTRPT1:
	int strt = 0;int stp = 0;int i=0; //counter
	String str = WEBSTRPT1;
	int len = str.length();
	...
	while(stp<len){
	   ...
	   stp=strt+szchnk;
	   if(stp>len){stp=len;}
	   if(DEBUG_MESSAGES){Serial.println("[handleRoot]"+String(i)+":");}
	   httpServer.sendContent(str.substring(strt,stp).c_str());
	   if(DEBUG_MESSAGES){Serial.println(str.substring(strt,stp));}
	}
	
	//Send JSON:
	strt = 0; stp = 0;
	str = "var fishyNetJSON ='" + getJSON() + "';";
	len = str.length();
	if(DEBUG_MESSAGES){Serial.println("[handleRoot] JSON str.len: "+String(len));}
	while(stp<len){
	   i++;
	   strt=stp;
	   stp=strt+szchnk;
	...
	str = WEBSTRPT2;
	len = str.length();
	if(DEBUG_MESSAGES){Serial.println("[handleRoot] PT2 str.len: "+String(len));}
	while(stp<len){
	   i++;
	   strt=stp;
	   stp=strt+szchnk;
	...
	httpServer.sendContent(""); // *** END 1/2 ***
	httpServer.client().stop(); // *** END 2/2 ***
	
	if(DEBUG_MESSAGES){Serial.println("[handleRoot]\n");}
...
String motPosStr(int intPadded, fishyDevice device){
	...
	}else return paddedInt(intPadded,0);
}
