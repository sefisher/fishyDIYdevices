...
|ESP8266|**Bounce2** by Thomas Fredericks|[PAGE](https://github.com/thomasfredericks/Bounce2)|[ZIP](https://github.com/thomasfredericks/Bounce2/archive/master.zip)|

### 2. Compile and upload the software to your device:
  #### a. Open "fishyDIYdevices.zip" and extract the example folder named "FD-Simple-Switch" from  (.zip > examples > LimitSwitchActuator > FD-Simple-Switch). You should find the following files in the extracted folder:
  - [FD-Simple-Switch.ino](FD-Simple-Switch/FD-Simple-Switch.ino) - this is the main file with loop() and setup() functions, it is generally the same flow for all fishyDIYdevices. It also contains all the definitions for the custom (device type specific) functions.
  - [FD-Simple-Switch.h](FD-Simple-Switch/FD-Simple-Switch.h) - this header adds declarations for functions, adds device-type specific variables, settings, and global variables.
  - [FD-Device-Definitions.h](FD-Simple-Switch/FD-Device-Definitions.h) - this header defines device settings for compiling (name, device-type, etc), it is common to all fishyDIYdevices. You can compile it as is and make the changes to the device settings via the web interface.
  #### b. Build and upload the file:
  - Open "FD-Simple-Switch.ino" in the Arduino IDE
